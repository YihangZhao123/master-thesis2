...
class channelSrc implements Template {
	...
	override create() {
		Global.model.vertexSet().stream().filter([v|v.hasTrait("impl::TokenizableDataBlock")]).forEach([ v |
			Save.save(srcPath(v), v.src())
		])
	...
	private def String srcPath(Vertex vertex) {
